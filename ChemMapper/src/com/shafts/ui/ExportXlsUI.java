...
public class ExportXlsUI extends JFrame{
		...
		OK.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(rButton1.isSelected())
					...
					if(i == 0||i > j){
						if(i == 0)
							JOptionPane.showMessageDialog( null,"The start number can't be 0 ！");
						else
							JOptionPane.showMessageDialog( null,"Input illegal! The start number should be smaller than the end number!");
					}
					else{
						exportXls(table,i,j);
					}
	...
	protected void exportXls(JTable table, int i, int j) {
		...
		if(table.getRowCount() == 1)
			JOptionPane.showMessageDialog( null,"There is no any data in the form！");
		else{
