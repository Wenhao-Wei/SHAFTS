 ...
 *  
 *  In SHAFTS, we self-defined this class for hope of getting some local features.
 *  Since the package and the class name is totally the same with the one in 
 *  jchempaint.jar, we need to configure the classpath of the project, and put the 
 *  src source folder in front of jchempaint.jar.
 */
...
public abstract class AbstractJChemPaintPanel extends JPanel{
    ...
    public void customizeView() {
        if (showMenuBar) {
            ...
            topContainer.add(menu, BorderLayout.NORTH); // set the File, Edit, View, Help etc. in the top-most...
        } else {
        ...
        if (showStatusBar) {
            ...
            add(statusBar, BorderLayout.SOUTH); // set the status bar in the button-most
        } else {
            remove(statusBar);
        }
        if (showToolBar) {
            ...
            if (uppertoolbar != null) {
                topContainer.add(uppertoolbar, BorderLayout.SOUTH); // the icon shortcut, like open, save, erase ...
            }
            if (lefttoolbar == null) {
                lefttoolbar = JCPToolBar.getToolbar(this, "lefttoolbar", SwingConstants.VERTICAL, blacklist);
            }
            if (lefttoolbar != null) {
                centerContainer.add(lefttoolbar, BorderLayout.WEST); // the valence bond icon shortcut
            }
            if (righttoolbar == null) {
                righttoolbar = JCPToolBar.getToolbar(this, "righttoolbar", SwingConstants.VERTICAL, blacklist);
            }
            if (righttoolbar != null) {
                centerContainer.add(righttoolbar, BorderLayout.EAST); // the relative ring structures icon shortcut
            }
            if (lowertoolbar == null) {
                lowertoolbar = JCPToolBar.getToolbar(this, "lowertoolbar", SwingConstants.HORIZONTAL, blacklist);
            }
            if (lowertoolbar != null) {
                centerContainer.add(lowertoolbar, BorderLayout.SOUTH); // the element shortcut
            }
        } else {
...
//            centerContainer.add(insertTextPanel, BorderLayout.NORTH); // insert SMILES text into the center panel
        } else {
