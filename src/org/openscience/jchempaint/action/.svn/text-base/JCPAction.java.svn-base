...
public class JCPAction extends AbstractAction
{
	...
	public AbstractJChemPaintPanel getJcpPanel() {
		return jcpPanel;
	}


	public void setJcpPanel(AbstractJChemPaintPanel jcpPanel) {
		this.jcpPanel = jcpPanel;
	}
	
	public void saveToRenderFile() {
		try {
			if (saveAsAction == null)
				saveAsAction = new SaveAsAction(getJcpPanel(), false);
			saveAsAction.saveRenderFileAsMol(getJcpPanel().getChemModel(), CreateMolecule.RENDER_FILE_NAME);
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	}
